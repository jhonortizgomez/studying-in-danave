    ...
    <section aria-labelledby="games-title" className="space-y-4">
      <Card className="border-0 bg-linear-to-br from-white/80 to-accent/5 backdrop-blur-sm">
        <CardHeader>
          <CardTitle
            id="games-title"
            className="flex items-center gap-3 text-2xl bg-linear-to-r from-accent via-primary to-purple bg-clip-text text-transparent"
          >
          ...
          {games.map((game) => (
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                <h3 className="font-bold text-foreground text-sm flex-1 group-hover:text-accent transition-colors">
                  {game.title}
                </h3>
                <Badge
                  variant={game.level === "A1" ? "level_a1" : "level_a2"}
                  className="shrink-0"
                >
                  {game.level}
                </Badge>
